...
#Region "Syspro Connections"
    ...
    Public Function GetSysproUserID(OperatorName As String, ConnectionString As String) As String
        ...
        If conn.DataSource = "LITZ-SYSPRODB" Then
            'production
            database = "use Sysprodb"
        Else
            'dev
            database = "use SysproDevDb"
        End If

        conn.Open()
        Dim company = conn.ExecuteScalar($"{database}
                                            select sa.Company
                                            from dbo.SysproAdmin sa
                                            where sa.DatabaseName = '{conn.Database}'").Trim

        sqlStr = $"{database}
                select ast.UserId, ast.Store1
                from dbo.AdmState ast
